...
def crawl_geo(suburl):
    ...
    details = soup.select('#__next > div.poiDetailPageWrap > div > div.moduleWrap > div.mainModule > div.detailModuleRef > div > div:nth-child(2) > div > div > p')    
    for item in details:
        detail_arr.append(item.get_text())

    for item in title_list:
        subps = item.find_all('p')
        if len(subps) ==2 :
            info = {'content': content, 'target': subps[1].get_text(), 'pred': subps[0].get_text(), 'envir': '网络教明明'}
